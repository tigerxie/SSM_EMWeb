...
<HTML>
	...
	<body>
		<!--  -->
		<form id="userAction_save_do" name="Form1" action="${pageContext.request.contextPath}/adminProduct/update.action" method="post" enctype="multipart/form-data">
		...
		<input type="hidden"  name="image" value='${product.image }'/>
			...
			<table cellSpacing="1" cellPadding="5" width="100%" align="center" bgColor="#eeeeee" style="border: 1px solid #8ba7e3" border="0">
				...
				<tr>
					...
					<td class="ta_01" bgColor="#ffffff">
						<select name="isHot">
							<option value="1" <c:if test="${product.isHot eq 1 }">selected</c:if>>是</option>
							<option value="0" <c:if test="${product.isHot eq 0 }">selected</c:if>>否</option>
						</select>
					</td>
				</tr>
				<tr>
					...
					<td class="ta_01" bgColor="#ffffff" colspan="3">
						<select name="csid">
							<c:forEach items="${csList }" var="cs">
								<option value="${cs.csid }" <c:if test="${product.csid eq cs.csid }">selected</c:if>>${cs.csname }</option>
							</c:forEach>
						</select>
					</td>
				</tr>
